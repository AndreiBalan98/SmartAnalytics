...
function GoogleAdsCallbackContent() {
  ...
  async function exchangeCode(code: string, redirectUri: string) {
    try {
      ...
      setMessage('Connecting to Google Ads...')

      const data = await api.exchangeGoogleAdsCode(code, redirectUri)

      setStatus('success')
      setMessage(`Successfully connected to Google Ads!${data.account_email ? ` Account: ${data.account_email}` : ''}`)

      // Close popup and notify parent if opened as popup
      if (window.opener) {
        window.opener.postMessage({ type: 'GOOGLE_ADS_OAUTH_SUCCESS' }, window.location.origin)
        setTimeout(() => {
          window.close()
        }, 1000)
      } else {
        ...
        setTimeout(() => {
          router.push('/agency/dashboard?google_ads_connected=true')
        }, 2000)
      }
    } catch (error: any) {
      setStatus('error')
      setMessage(error.message || 'Failed to connect Google Ads')
